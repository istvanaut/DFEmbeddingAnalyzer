...
realVideos = df[df.target == 0].shape[0]
fakeVideos = df[df.target == 1].shape[0]

print("Real: ", df[df.target == 0].shape, "Fake: ", df[df.target == 1].shape)
...
for currentModel in ["RF", "SVC", "LR"]:
    ...
    while currentRound < 5:
        ...
        if currentModel == "RF":
            model = RandomForestClassifier()
        elif currentModel == "SVC":
            model = SVC()
        elif currentModel == "LR":
        ...
        plt.title(str(currentModel) + ", Score " + str(score) + ", Real: " + str(realVideos) + ", Fake: " + str(fakeVideos))
        plt.xlabel("Predicted")
        plt.ylabel("Truth")
        plt.savefig(str(currentModel) + "_score_" + str(score) + "_real_" + str(realVideos) + "_fake_" + str(fakeVideos) + "_" + str(currentRound) + ".png")
